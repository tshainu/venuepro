...
if ($booking_id) {
    ...
    if ($booking) {
        if ($booking['package_price']) $prefill_items[] = ['description'=>$booking['package_name'].' (Package)','quantity'=>1,'unit_price'=>$booking['package_price'],'tax_percent'=>0,'total'=>$booking['package_price']];
        $ba = $db->fetchAll("SELECT * FROM booking_addons WHERE booking_id=?", [$booking_id]);
        foreach ($ba as $a) $prefill_items[] = ['description'=>$a['name'],'quantity'=>$a['quantity'],'unit_price'=>$a['unit_price'],'tax_percent'=>$a['tax_percent'],'total'=>$a['total_price']];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $invoice_type  = $_POST['invoice_type'] ?? 'advance';
...
$breadcrumbs = [['label'=>'Invoices','url'=>BASE_URL.'/modules/invoices/index.php'],['label'=>'New']];
require_once ROOT_PATH . '/includes/header.php';
?>
<div class="vp-page-header d-print-none">
  <div class="d-flex align-items-center"><h1 class="vp-page-title">New Invoice</h1></div>
</div>
<form method="post">
  ...
  <div class="col-lg-8">
    <div class="card vp-card mb-3">
      ...
      <div class="card-body">
        ...
        <?php if ($booking): ?><div class="alert alert-info mb-3">Booking: <strong><?= Helper::sanitize($booking['booking_ref']??'') ?></strong> — <?= Helper::sanitize($booking['customer_name']) ?></div><?php endif; ?>
    ...
    <div class="card vp-card mb-3">
      <div class="card-body">
        <div class="mb-3"><label class="form-label">Notes</label><textarea name="notes" class="form-control" rows="2"><?= Helper::sanitize($_POST['notes']??'') ?></textarea></div>
        <div class="mb-3"><label class="form-label">Terms &amp; Conditions</label><textarea name="terms" class="form-control" rows="3"><?= Helper::sanitize($_POST['terms']??$def_terms) ?></textarea></div>
      </div>
    </div>
  </div>
  ...
  <div class="col-lg-4">
    <div class="card sticky-top" style="top:80px">
      ...
      <div class="card-body">
        ...
        <div class="mb-2"><label class="form-label">Discount (Rs.)</label><input type="number" name="discount_amount" id="discount_input" class="form-control" step="0.01" min="0" value="<?= $_POST['discount_amount']??0 ?>"></div>
        <hr>
        <div class="d-flex justify-content-between fw-bold fs-5"><span>Total</span><span id="sum-total">Rs. 0.00</span></div>
      </div>
      <div class="card-footer"><button type="submit" class="btn btn-primary w-100">Create Invoice</button></div>
    </div>
  </div>
</div>
</form>
...
function recalc(){let sub=0,tax=0;document.querySelectorAll('.item-row').forEach(row=>{const qty=parseFloat(row.querySelector('.item-qty').value)||0,price=parseFloat(row.querySelector('.item-price').value)||0,taxp=parseFloat(row.querySelector('.item-tax').value)||0,line=qty*price;sub+=line;tax+=line*taxp/100;row.querySelector('.item-total').textContent='Rs. '+line.toFixed(2);});const disc=parseFloat(document.getElementById('discount_input').value)||0;document.getElementById('sum-sub').textContent='Rs. '+sub.toFixed(2);document.getElementById('sum-tax').textContent='Rs. '+tax.toFixed(2);document.getElementById('sum-total').textContent='Rs. '+Math.max(0,sub+tax-disc).toFixed(2);}
recalc();
</script>
