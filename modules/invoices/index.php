...
$stat_where = "1=1";
if (!empty($accessible_branches)) {
    $stat_where = "i.branch_id IN (" . implode(',', array_map('intval', $accessible_branches)) . ")";
} else {
    $stat_where = "0=1";
}
$statusStats = $db->fetchAll("SELECT i.status, COUNT(*) as cnt, COALESCE(SUM(i.total),0) as total, COALESCE(SUM(i.balance),0) as balance FROM invoices i WHERE $stat_where GROUP BY i.status");
$stMap = []; foreach ($statusStats as $s) $stMap[$s['status']] = $s;

// Unpaid bookings (balance > 0, not cancelled, no active unpaid invoice)
$bk_cond = "";
if (!empty($accessible_branches)) {
    $bk_cond = "AND b.branch_id IN (" . implode(',', array_map('intval', $accessible_branches)) . ")";
} else {
    $bk_cond = "AND 0=1";
}
