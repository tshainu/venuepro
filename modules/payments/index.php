...
if (!empty($accessible_branches)) {
    ...
    $params = array_merge($params, $accessible_branches);
} else {
    $where[] = "0=1"; // No branches accessible
}
...
$pb_cond = "";
if (!empty($accessible_branches)) {
    $pb_cond = "AND b.branch_id IN (" . implode(',', array_map('intval', $accessible_branches)) . ")";
} else {
    $pb_cond = "AND 0=1";
}
...
$method_cond = "WHERE 1=1";
if (!empty($accessible_branches)) {
    $method_cond = "WHERE p.branch_id IN (" . implode(',', array_map('intval', $accessible_branches)) . ")";
} else {
    $method_cond = "WHERE 0=1";
}
$method_breakdown = $db->fetchAll("SELECT p.payment_method, COUNT(*) as cnt, SUM(p.amount) as total FROM payments p $method_cond GROUP BY p.payment_method ORDER BY total DESC");
