...
$event_types = ['Wedding','Engagement','Wedding Reception','Puberty','Birthday','Corporate','Conference','Get Together','Other'];
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $bride_dob_day  = (int)($_POST['bride_dob_day'] ?? 0);
    $bride_dob_month= (int)($_POST['bride_dob_month'] ?? 0);
    $bride_dob      = ($bride_dob_day && $bride_dob_month) ? $bride_dob_day.'/'.$bride_dob_month : '';
    $groom_dob_day  = (int)($_POST['groom_dob_day'] ?? 0);
    $groom_dob_month= (int)($_POST['groom_dob_month'] ?? 0);
    $groom_dob      = ($groom_dob_day && $groom_dob_month) ? $groom_dob_day.'/'.$groom_dob_month : '';
    $hero_dob_day   = (int)($_POST['hero_dob_day'] ?? 0);
    $hero_dob_month = (int)($_POST['hero_dob_month'] ?? 0);
    $hero_dob       = ($hero_dob_day && $hero_dob_month) ? $hero_dob_day.'/'.$hero_dob_month : '';
...
function openCustomerHistory() {
  ...
  container.innerHTML = '<div class="text-center py-5"><div class="spinner-border text-primary"></div><p class="mt-2 text-muted">Loading history...</p></div>';
  modal.show();
  
  fetch('customer_history.php?customer_id=' + cid)
    .then(r => r.text())
    .then(html => {
      container.innerHTML = html;
    })
    .catch(err => {
      container.innerHTML = '<div class="alert alert-danger m-3">Error loading history.</div>';
    });
}
    ...
    <div class="evt-card">
      ...
      <div class="evt-card-body">
        <div class="evt-type-grid">
          <?php
          $typeIcons = ['Wedding'=>'💍','Engagement'=>'💑','Wedding Reception'=>'🥂','Puberty'=>'👧','Birthday'=>'🎂','Corporate'=>'🏢','Conference'=>'🎙️','Get Together'=>'🎊','Other'=>'📋'];
        ...
        <div class="evt-names-panel <?= in_array($evtType,['Wedding','Engagement','Wedding Reception'])?'show':'' ?>" id="wedding-names-panel">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              <label class="form-label">🎂 Bride's Date of Birth <span style="font-size:.75rem;color:#9ca3af;">(Day &amp; Month only)</span></label>
              <div class="d-flex gap-2">
                <select name="bride_dob_day" class="form-select">
                  <option value="">Day</option>
                  <?php for($d=1;$d<=31;$d++): $sel=((int)explode('/',$_POST['bride_dob']??'0/0')[0]===$d)?'selected':''; ?>
                  <option value="<?= $d ?>" <?= $sel ?>><?= $d ?></option>
                  <?php endfor; ?>
                </select>
                <select name="bride_dob_month" class="form-select">
                  <option value="">Month</option>
                  <?php $months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; foreach($months as $mi=>$mn): $sel=((int)explode('/',$_POST['bride_dob']??'0/0')[1]===($mi+1))?'selected':''; ?>
                  <option value="<?= $mi+1 ?>" <?= $sel ?>><?= $mn ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
            </div>
            <div class="col-md-6">
              <label class="form-label">🎂 Groom's Date of Birth <span style="font-size:.75rem;color:#9ca3af;">(Day &amp; Month only)</span></label>
              <div class="d-flex gap-2">
                <select name="groom_dob_day" class="form-select">
                  <option value="">Day</option>
                  <?php for($d=1;$d<=31;$d++): $sel=((int)explode('/',$_POST['groom_dob']??'0/0')[0]===$d)?'selected':''; ?>
                  <option value="<?= $d ?>" <?= $sel ?>><?= $d ?></option>
                  <?php endfor; ?>
                </select>
                <select name="groom_dob_month" class="form-select">
                  <option value="">Month</option>
                  <?php $months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; foreach($months as $mi=>$mn): $sel=((int)explode('/',$_POST['groom_dob']??'0/0')[1]===($mi+1))?'selected':''; ?>
                  <option value="<?= $mi+1 ?>" <?= $sel ?>><?= $mn ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
        ...
        <div class="evt-names-panel <?= !in_array($evtType,['Wedding','Engagement','Wedding Reception']) && $evtType ? 'show' : '' ?>" id="hero-name-panel">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              <label class="form-label">🎂 Date of Birth <span style="font-size:.75rem;color:#9ca3af;">(Day &amp; Month only)</span></label>
              <div class="d-flex gap-2">
                <select name="hero_dob_day" class="form-select">
                  <option value="">Day</option>
                  <?php for($d=1;$d<=31;$d++): $sel=((int)explode('/',$_POST['hero_dob']??'0/0')[0]===$d)?'selected':''; ?>
                  <option value="<?= $d ?>" <?= $sel ?>><?= $d ?></option>
                  <?php endfor; ?>
                </select>
                <select name="hero_dob_month" class="form-select">
                  <option value="">Month</option>
                  <?php $months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; foreach($months as $mi=>$mn): $sel=((int)explode('/',$_POST['hero_dob']??'0/0')[1]===($mi+1))?'selected':''; ?>
                  <option value="<?= $mi+1 ?>" <?= $sel ?>><?= $mn ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
      ...
      <div class="evt-card-body">
        ...
        <div class="customer-card-preview" id="cust-preview">
          ...
          <div class="ms-auto d-flex align-items-center gap-2">
            ...
            <span class="cust-info-badge">✓ Selected</span>
          </div>
        </div>
      </div>
    </div>
...
function showCustomerPreview(sel) {
  ...
  if (!opt.value) { 
    ...
    const histBtn = document.getElementById('btn-cust-history');
