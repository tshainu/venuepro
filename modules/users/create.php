...
if (!$branch_id && !Auth::isSuperAdmin()) {
    Helper::redirect(BASE_URL . '/index.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        ...
        $role = $db->fetchOne("SELECT slug FROM roles WHERE id = ? AND slug IN ('reception', 'accountant', 'hall_manager', 'manager', 'receptionist', 'staff')", [$role_id]);
        if (!$role) {
    ...
    if (!$error) {
        // Check duplicates — email and username must be unique
        ...
        try {
            ...
            $db->execute(
                "INSERT INTO users (name, email, user_id, username, password, phone, role_id, branch_id, is_active, created_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, NOW())",
                [$name, $email, $user_id, $username, $hashed, $phone, $role_id, $selected_branches[0]]
            );
            $new_uid = $db->fetchOne("SELECT LAST_INSERT_ID() as id")['id'] ?? null;

            // Save branches
            foreach ($selected_branches as $bid) {
                $db->execute("INSERT INTO user_branches (user_id, branch_id) VALUES (?, ?)", [$new_uid, $bid]);
            }

            $db->commit();
            Logger::log('create', 'users', (int)$new_uid, $username, null,
                ['name'=>$name,'email'=>$email,'username'=>$username,'role_id'=>$role_id,'branches'=>$selected_branches],
                "Created user $name ($username) with access to " . count($selected_branches) . " branches");
              ...
              <?php foreach ($branches as $b): ?>
                <label style="display: flex; align-items: center; gap: .5rem; font-size: .85rem; cursor: pointer;">
                  <input type="checkbox" name="branch_ids[]" value="<?= $b['id'] ?>" 
                         <?= in_array($b['id'], $_POST['branch_ids'] ?? [$branch_id]) ? 'checked' : '' ?>>
