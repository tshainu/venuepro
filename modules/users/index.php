...
if (($_GET['action'] ?? null) === 'delete' && ($_GET['id'] ?? null)) {
    ...
    if (Auth::isSuperAdmin()) {
        $hasPermission = true;
    } elseif ($sa_biz_id && $userCheck) {
        // Check if user belongs to the same business
        $hasPermission = ($userCheck['user_id'] === strtoupper($user_uid_to_check));
    } elseif ($userCheck && $userCheck['branch_id'] == $branch_id) {
        $hasPermission = true;
    }

    if ($hasPermission && $userCheck) {
...
if (Auth::isSuperAdmin()) {
    $staff = $db->fetchAll(
        "SELECT u.id, u.name, u.email, u.user_id, u.username, u.phone, u.is_active, u.created_at, r.name as role_name
         ...
         WHERE u.role_id != (SELECT id FROM roles WHERE slug = 'super_admin')
    ...
    $staff = $db->fetchAll(
        "SELECT u.id, u.name, u.email, u.user_id, u.username, u.phone, u.is_active, u.created_at, r.name as role_name
         FROM users u
         LEFT JOIN roles r ON u.role_id = r.id
         WHERE u.user_id = ? AND u.role_id != (SELECT id FROM roles WHERE slug = 'super_admin')
         ORDER BY u.created_at DESC",
        [strtoupper($user_uid_to_check)]
    );
} else {
    ...
    if (!empty($accessible_branches)) {
        ...
        $staff = $db->fetchAll(
            "SELECT u.id, u.name, u.email, u.user_id, u.username, u.phone, u.is_active, u.created_at, r.name as role_name
             FROM users u
             LEFT JOIN roles r ON u.role_id = r.id
             WHERE u.branch_id IN ($placeholders) AND u.role_id != (SELECT id FROM roles WHERE slug = 'super_admin')
             ORDER BY u.created_at DESC",
            $accessible_branches
        );
    } else {
