...
class Auth {
    ...
    private function setSession($user) {
        ...
        $accessible_branches = $this->db->fetchAll(
            "SELECT branch_id FROM user_branches WHERE user_id = ?",
            [$user['id']]
        );
        $_SESSION['accessible_branches'] = array_column($accessible_branches, 'branch_id');
    ...
    public static function check() {
        ...
        if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] === '' || !isset($_SESSION['branch_id'])) {
            ...
            // Load all accessible branches
            $accessible_branches = $db->fetchAll(
                "SELECT branch_id FROM user_branches WHERE user_id = ?",
                [$_SESSION['user_id']]
            );
            $_SESSION['accessible_branches'] = array_column($accessible_branches, 'branch_id');
    ...
    public static function getAccessibleBranches() {
        if (self::isSuperAdmin()) {
            $db = Database::getInstance();
            $branches = $db->fetchAll("SELECT id FROM branches WHERE is_active = 1");
